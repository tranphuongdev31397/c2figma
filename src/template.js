...
async function main() {
  ...
  for (const item of SCENE.nodes) { const parent=nodes.get(item.parentId)||root, pb=bounds.get(item.parentId)||bounds.get('__root__'), node=item.kind==='text'?figma.createText():item.kind==='svg'?figma.createNodeFromSvg(item.svg):figma.createFrame(); node.name=item.name||item.kind; node.x=item.x-pb.x; node.y=item.y-pb.y; node.resize(Math.max(1,item.width),Math.max(1,item.height)); node.opacity=item.opacity ?? 1;
    ...
    else if(item.kind!=='svg'){node.layoutMode='NONE';node.fills=item.fill?[solid(item.fill)]:[];const b=item.borders,w=b?{top:b.top.width,right:b.right.width,bottom:b.bottom.width,left:b.left.width}:{top:item.strokeWidth,right:item.strokeWidth,bottom:item.strokeWidth,left:item.strokeWidth},p=b?[b.top,b.right,b.bottom,b.left].find(s=>s.width)?.color:item.stroke;if(p&&Math.max(w.top,w.right,w.bottom,w.left)){node.strokes=[solid(p)];node.strokeWeight=Math.max(w.top,w.right,w.bottom,w.left);node.strokeTopWeight=w.top;node.strokeRightWeight=w.right;node.strokeBottomWeight=w.bottom;node.strokeLeftWeight=w.left;}if(item.radius)node.cornerRadius=item.radius;}
