(function (global) {
  ...
  function captureWhenStable(serialize, token, width, height, minimumDelay) {
    ...
    const check = () => {
      ...
      // looks still is not settled until nothing is animating on it
      const animating = document.getAnimations && document.getAnimations().some(animation => animation.playState === 'running');
  ...
  function interactionToolkit(actionKeyFor, minimumDelay, settleMs) {
    ...
    // straight into the fade that starts right after. Only a page still calm on a second look is calm.
    const settleAfterAction = async () => {
      let calm = 0;
      for (let attempt = 0; attempt < 8 && calm < 2; attempt += 1) {
